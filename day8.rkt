...
; An InputLine is a (list (list Str Str Str Str Str Str Str Str Str Str) [List-of Str]

; is-unique-option?: Str -> (U Number Boolean)
(define (is-unique-option? str)
...
; part1: [List-of InputLine] -> Nat
(define (part1 loil)
  (for/sum ([line loil])
    (match line
      [(list _ out)
       (for/sum ([num out]
                 #:when (is-unique-option? num))
         1)])))

; all-possible-options : Number -> [List-of Nat]
(define (all-possible-options num)
  (cond [(zero? num) '(0 1 2 4 5 6)]
        [(= 1 num) '(2 5)]
        [(= 2 num) '(0 2 3 4 6)]
        [(= 3 num) '(0 2 3 5 6)]
        [(= 4 num) '(1 2 3 5)]
...
; one-deduction: Str [Hash Str Nat] -> [List-of Natural]
(define (one-deduction str)
  (let ((unique (is-unique-option? str))
        (length (string-length str)))
    (cond [(number? unique) (list unique)]
          [(= length 5) '(2 3 5)]
          [(= length 6) '(0 6 9)])))

; update-hash-with-deduction : Str Nat [Hash Str Nat] -> [List-of [Hash Str Nat]]
(define (update-hash-with-deduction str num hash)
  (define all-options (all-possible-options num))
  (map
   first
   (for/fold ([valid-hashes-l (list (list hash all-options))])
             ([char (map string (string->list str))])
     (for/fold ([hashes '()])
               ([valid-hash-l valid-hashes-l])
       (define valid-hash (first valid-hash-l))
       (define left-options (second valid-hash-l))
       (define cache-hit (hash-ref valid-hash char #f))
       (cond [(and cache-hit (member cache-hit left-options))
              (cons (list valid-hash (remove cache-hit left-options)) hashes)]
             [cache-hit
              hashes]
             [else
              (append hashes (for/fold ([acc '()])
                                       ([option left-options])
                               (if (member option (hash-values valid-hash))
                                   acc
                                   (cons (list (hash-set valid-hash char option)
                                               (remove option left-options))
                                         acc))
                               ))])))))

; try-deduction: [List-of Str] [Hash Str Nat] -> (U #f [Hash Str Nat])
(define (try-deduction nums previous)
  (match nums
    [(list first rest ...)
     (for/fold ([val #f])
               ([deduction (one-deduction first)])
       (or val
           (for/fold ([up #f])
                     ([new-hash (update-hash-with-deduction first
                                                            deduction
                                                            previous)])
             (or up
                 (try-deduction rest new-hash)))))]
    [_ (and (= 7 (length (hash->list previous))) previous)]))

; list-unorder=? : [List-of X] [List-of X] -> Boolean
(define (list-unorder=? l1 l2)
  (equal? (list->set l1) (list->set l2)))

; get-values: [List-of String] [Hash Str Int] -> Int
(define (get-values los h)
  (for/fold ([acc 0])
            ([str los])
    (define options
      (for/list ([char (string->list str)])
        (hash-ref h (string char))))
    (+ (* 10 acc)
       (for/fold ([acc #f])
                 ([num 10])
         (or acc
             (and (list-unorder=? options (all-possible-options num))
                  num))))))

; perform-deduction : Line -> Int
(define (perform-deduction line)
  (match line
    [(list guesses output)
     (let ((deduction (try-deduction guesses (make-immutable-hash))))
       (get-values output deduction))
     ]))

; part2 : [List-of Line] -> Int
(define (part2 lol)
  (for/sum ([line lol])
    (perform-deduction line)))
...
(define INPUT (for/list ([line prev-input])
                (list (map symbol->string (first line)) (map symbol->string (second line)))))
(define one-line '((("acedgfb" "cdfbe" "gcdfa" "fbcad" "dab" "cefabd" "cdfgeb" "eafb" "cagedb" "ab") 
